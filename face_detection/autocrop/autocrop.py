...
class Cropper:
    ...
    * `fix_gamma`: `bool`, default=`True`
    ...
    def __init__(
        self, width=500, height=500, face_percent=50, padding=None, fix_gamma=True, smoother=False,
    ):
        ...
        label_path = os.path.join(directory, "voc-model-labels.txt")
        class_names = [name.strip() for name in open(label_path).readlines()]
        model_path = os.path.join(directory, "version-RFB-640.pth")
        net = create_Mb_Tiny_RFB_fd(len(class_names), is_test=True, device="cuda")
        self.predictor = create_Mb_Tiny_RFB_fd_predictor(net, candidate_size=1500, device="cuda")
        net.load(model_path)
    ...
    def crop(self, path_or_array):
        ...
        boxes, _, _ = self.predictor.predict(gray, 1500 / 2, 0.6)
        if len(boxes) == 0:
            return None
            ...
            boxes.sort(key=lambda i: (i[2] - i[0]) + (i[3] - i[1]), reverse=True)
            box = boxes[0]
        ...
        # print(pos)
        if self.smoother:
            box[1] = max(0, box[1] - self.amount)  # y1
            box[0] = max(0, box[0] - self.amount)  # x1
            box[3] = min(img_height, box[3] + self.amount)  # y2
            box[2] = min(img_width, box[2] + self.amount)  # x2
            new_width = box[2] - box[0]
            new_height = box[3] - box[1]
            diff = abs(new_width - new_height)
            if new_height < new_width:
                box[1] = max(0, box[1] - diff / 2)
                box[3] = min(img_height, box[3] + diff / 2)
            else:
                box[0] = max(0, box[0] - diff / 2)
                box[2] = min(img_width, box[2] + diff / 2)
